...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request): JsonResponse
    {
        try {
            ...
            // check role and current JWT token
            if ($user->role === 'user') {
                if ($user->current_jwt_token && $user->current_jwt_token !== $token) {
                    return response()->json([
                        'status' => 'error',
                        'message' => 'Anda telah mencapai batas perangkat untuk login.',
                    ], 403)->header('Content-Type', 'application/json');
                }
            }

            // save the current JWT token
